// tools/build.js
// Reads data/scheme.txt, parses it like your browser generator, bakes Base64 CSV into DispatchTool.html

const fs = require('fs');
const path = require('path');

// ------- helpers copied from your generator -------
...
function isDigits(str, count) {
  ...
  for (let i = 0; i < str.length; i++) { const ch = str.charCodeAt(i); if (ch < 48 || ch > 57) return false; }
  return true;
}
function normalizeTokenToRange(token) {
  ...
  if (dash !== -1) {
    ...
    if (isDigits(A, 3) && isDigits(B, 3)) {
      const e = expand3To5(A, B), s = e[0], t = e[1];
    ...
    if (isDigits(token, 3)) {
      const e2 = expand3To5(token, token), s2 = e2[0], t2 = e2[1];
...
function parseSchemeToRows(text) {
  const rows = []; let txt = String(text || "");
  ...
  for (let k = 0; k < rows.length; k++) {
    ...
    if (seen[key]) continue; seen[key] = true; out.push(r);
  }
...
function b64encodeUtf8(str) {
  return Buffer.from(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, (_, p1) => String.fromCharCode('0x' + p1)), 'binary').toString('base64');
}

// Build the same single-file tool you generate in the browser
function buildDispatchHTML(b64csv) {
  // Keep this in sync with your generator. Copied as-is, minor tweaks for Node safety.
  let html = '';
...
if (!fs.existsSync(schemePath)) {
  console.error('data/scheme.txt not found. Did the workflow extract the issue body?');
  process.exit(1);
}
